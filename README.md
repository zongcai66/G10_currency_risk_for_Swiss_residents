...
3. run the command to start all containers:
   ...
   docker-compose down
   ```
5. if you want to start containers one by one(please copy one by one):
   ```bash
...
6. I have builded volumes for Automatically mapping files to local, so you can find your reports in the local files or you can go the docker desktop-->containers-->paper-builder-->Bind mounts-->click link
   or you can:
   ```bash
...
1. you can pull my docker i have builded in the docker hub:
   ```bash
   docker pull wz0973/g10_currency_analysis:latest
   ```
   and run in the docker desktop, and check the file following the way i wrote above.
   But I suggest that you'd better download the code locally instead of this method.


# Project Guide(building docker from clonging github code)
...
1. Ensure you are in the project directory:
   ...
   ```bash
   docker build -t wz0973/g10_currency_analysis:latest .
   ```
